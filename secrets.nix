let
  ...
  # secret. Its private half is backed up off-machine, so losing the
  # machine and the NAS together still leaves a recovery path (see
  # docs/secrets.md). (GitHub pushes use a separate dedicated key,
  # ~/.ssh/git — see modules/batman/ssh.nix.)
  batman =
